...
# Fires when any file inside .dsh/skills/ is written or edited.
#
# Exit behavior:
...
# Only act on files inside .dsh/skills/
if ! echo "$FILE_PATH" | grep -qE '(^|/)\.dsh/skills/'; then
    exit 0
fi

# Extract skill name from path (.dsh/skills/[skill-name]/SKILL.md)
SKILL_NAME=$(echo "$FILE_PATH" | grep -oE '\.dsh/skills/[^/]+' | sed 's|\.dsh/skills/||')
