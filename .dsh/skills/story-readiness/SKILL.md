...
**Scope:** determine from the task (see below); if no scope is implied, ask the user
via `ask_user_question`.

- **Specific path** (e.g., `production/epics/combat/story-001-basic-attack.md`):
...
- **No implied scope**: ask the user which scope to validate.

If no scope is implied, use `ask_user_question`:
...
Spawn `qa-lead` via subagent using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
