...
spawn `creative-director` via subagent:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via subagent in parallel:
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via subagent in parallel:
...
**Agent delegation (MANDATORY)**: Spawn `qa-lead` via subagent before finalising acceptance criteria. Provide: the completed GDD sections C, D, E, and ask them to validate that the criteria are independently testable and cover all core rules and formulas. Surface any gaps or untestable criteria to the user.
...
For required systems: **spawn `art-director` via subagent** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via subagent using gate **CD-GDD-ALIGN** (`.claude/docs/director-gates.md`).
