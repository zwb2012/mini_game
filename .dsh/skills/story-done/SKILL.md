...
- **Test pass check**: if a test file path is mentioned, run it via `pwsh`.
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via subagent using gate **QL-TEST-COVERAGE** (`.claude/docs/director-gates.md`).
...
Spawn `lead-programmer` via subagent using gate **LP-CODE-REVIEW** (`.claude/docs/director-gates.md`).
