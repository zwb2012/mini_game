...
**After dependency mapping is approved, spawn `technical-director` via subagent using gate TD-SYSTEM-BOUNDARY (`.claude/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via subagent using gate PR-SCOPE (`.claude/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via subagent using gate CD-SYSTEMS (`.claude/docs/director-gates.md`).**
...
After `/design-system` completes, use `ask_user_question`:
