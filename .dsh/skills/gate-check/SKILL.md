...
**Target phase:** determine from the task. If no target phase is implied, auto-detect
the current stage and validate the next transition.
...
- **Target phase given in the task**: validate readiness for that specific phase
  (e.g. the task says "gate to Production").
- **No target phase given**: Auto-detect the current stage using the same heuristics as
  the `project-stage-detect` skill, then **confirm with the user before running**:
...
**Quality Checks:**
- [ ] Tests are passing (run test suite via pwsh)
...
### Quality Checks
- For test checks: Run the test suite via `pwsh` if a test runner is configured
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via subagent using the parallel gate protocol from `.claude/docs/director-gates.md`. Issue all four subagent calls simultaneously — do not wait for one before starting the next.
