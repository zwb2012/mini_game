...
**Audit mode:** determine the mode from the task (default `full`). If the task does
not state a mode, use `full` or confirm via `ask_user_question`.

- **`full` (default)**: Complete audit — all artifact types
