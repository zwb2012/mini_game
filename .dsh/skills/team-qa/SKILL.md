...
Spawn `qa-lead` via subagent to review all in-scope stories and produce a QA strategy.
...
Spawn `qa-tester` via subagent for each story (run in parallel where possible), providing:
- The story file path
...
After each FAIL result: use `ask_user_question` to collect the failure description, then spawn `qa-tester` via subagent to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via subagent to produce the sign-off report using all results from Phases 4–6.
