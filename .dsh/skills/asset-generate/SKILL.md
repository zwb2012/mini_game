...
If no argument is provided, use `ask_user_question`:
...
```bash
python .dsh/skills/asset-generate/tools/slice_image.py \
  --input "[output_path]" \
...
**Requires `.dsh/skills/asset-generate/tools/generate_liblib.py`** — HMAC-SHA1 signature auth cannot be done with curl alone.

```bash
python .dsh/skills/asset-generate/tools/generate_liblib.py \
  --prompt "[prompt]" \
  --output "[output_path]" \
...
```bash
python .dsh/skills/asset-generate/tools/generate_audio.py \
