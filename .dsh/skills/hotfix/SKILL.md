...
`pwsh: git rev-parse --is-inside-work-tree 2>/dev/null`
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via subagent with:
