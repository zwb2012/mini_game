...
Hooks 通过 DSH 的 `hooks-claude-code` 桥接从 `.dsh/hooks.json` 配置。启用后会自动触发：
...
| `validate-skill-change.sh` | PostToolUse (write/edit) | Skill file changes | 在写入或编辑任何 `.dsh/skills/` 文件后，建议运行 `/skill-test` |
